...
func TestDefaultFormatter(t *testing.T) {
	out := DefaultFormatter(LOG_ERR, time.Now(), "hostname", "tag", "content", "")
...
func TestDefaultFormatterWithToken(t *testing.T) {
	out := DefaultFormatterWithToken(LOG_ERR, time.Now(), "hostname", "tag", "content", "my-token")
	expected := fmt.Sprintf("%s <%d> %s %s %s[%d]: %s",
		"my-token", LOG_ERR, time.Now().Format(time.RFC3339), "hostname", "tag", os.Getpid(), "content")
	if out != expected {
		t.Errorf("expected %v got %v", expected, out)
	}
}
...
func TestUnixFormatter(t *testing.T) {
	out := UnixFormatter(LOG_ERR, time.Now(), "hostname", "tag", "content", "")
...
func TestRFC3164Formatter(t *testing.T) {
	out := RFC3164Formatter(LOG_ERR, time.Now(), "hostname", "tag", "content", "")
	expected := fmt.Sprintf("<%d>%s %s %s[%d]: %s",
...
func TestRFC5424Formatter(t *testing.T) {
	out := RFC5424Formatter(LOG_ERR, time.Now(), "hostname", "tag", "content", "")
	expected := fmt.Sprintf("<%d>%d %s %s %s %d %s - %s",
		LOG_ERR, 1, time.Now().Format(time.RFC3339), "hostname", "tag",
